...
### Streamlit UI with Full-Length mode

As of v1.1, the Streamlit dashboard has two modes (toggle in sidebar):

**Quick clip (15-60s):** the existing mode — pick clip, analyze, view live in player.

**Full-Length (complete bout):** workflow:

1. Pick video in sidebar
2. Toggle to "Full-Length (komplettes Gefecht)"
3. Stay on "Konfiguration" tab
4. Fill in fencer A + B metadata (slug, name, nation, hand, club)
5. Enter bout data (tournament, date, score, weapon)
6. Choose output options (PDF, HD video, highlights, eval)
7. Click "🚀 Full-Length-Analyse starten"
8. Live log shows progress (refreshes every 5s)
9. After completion: open "DB durchsuchen" tab → see all saved bouts
10. Select bout → view metrics + annotations + output files

The UI spawns `analyze_full.py` as subprocess, polls status every 5s, auto-loads bout data from SQLite when done.
...
The Streamlit UI has Selenium tests in `tests/`. They start the app in a subprocess + Chrome headless, verify DOM and capture screenshots.
...
- Mode switch (Schnell-Clip / Full-Length) visible
- Video sources (Upload / Path / YouTube) visible
- Sidebar sections (Analyse-Modus, Video-Quelle)
- GPU/CPU indicator
- Screenshot capture for visual reviews
- No console errors
- Streamlit standard DOM (data-testid) correct

Note: tests start a real Streamlit server (port 8511) and Chrome. Per-test time: 5-30s.
...
### v1.3.2 (June 2026) — Refactoring: Logging, Efficiency, Tests
- **NEW:** `video_utils.py` — shared ffmpeg/ffprobe helpers (probe_video, extract_subclip, has_ffmpeg, has_ffprobe)
- **NEW:** Frame-range chunking — `scheduler.py` passes `--start-frame`/`--end-frame`; `worker_chunk_analyze.py` extracts only the subclip
- **NEW:** `tests/test_video_pipeline.py` + `tests/test_scheduler_integration.py` + `tests/mock_worker_chunk.py`
- **NEW:** `tests/test_eval_runner.py` — parser tests for `SCORE:/ISSUES:/SUGGESTIONS:`
- **NEW:** `tests/test_worker_frame_range.py` — performance benchmark (slow, `--runslow`)
- **REFACTOR:** replaced `print()` with `logging` in `analyze_full.py`, `scheduler.py`, `pause_detector.py`, `worker_chunk_analyze.py`
- **REFACTOR:** modularized `app.py` into `ui_media_server.py` + `ui_full_length.py`
- **FIX:** `eval_runner.py` parser now correctly handles inline single-line, multiline, mixed and missing sections
- All 19 tests passing (11 UI + 6 pipeline + 2 scheduler integration)

### v1.3.1 (June 2026) — Shared Video Utils
- **NEW:** `video_utils.py`: `probe_video`, `extract_subclip`, `has_ffmpeg`, `has_ffprobe`
- **UPDATE:** `pause_detector.py` and `worker_chunk_analyze.py` use shared video helpers
- **UPDATE:** `scheduler.py` reads fps from `PauseDetector.fps`

### v1.3 (June 2026) — Subagent Eval Loop + Selenium UI Tests
- **NEW:** `eval_runner.py` — Hermes integration with `delegate_task`
- **NEW:** Robust `SCORE:/ISSUES:/SUGGESTIONS:` parser (single-line, multi-line, mixed)
- **NEW:** `eval_runner.py` CLI — `python eval_runner.py reports/merged_*.json`
- **NEW:** `tests/conftest.py` + `tests/test_app_ui.py` — 11 Selenium UI tests
- **NEW:** `requirements-test.txt` — separate test dependencies
- **NEW:** `pytest.ini` — pytest config
- **NEW:** Skill `fencing-pose-analysis` v2.2 with eval_runner docs
- Tests verify: app load, mode switch, video sources, sidebar, GPU/CPU, screenshots, console errors
- Test run: `pytest tests/test_app_ui.py -v` (~3 min, headless Chrome)

### v1.2 (June 2026) — Streamlit UI Full-Length Mode
- **NEW:** `app.py` mode switch (Quick clip vs. Full-Length) in sidebar
- **NEW:** Full-Length configuration form (fencer A/B, bout metadata, output options)
- **NEW:** Subprocess wrapper calls `analyze_full.py` with all flags
- **NEW:** Live log fragment (5s polling) shows analysis progress
- **NEW:** "DB durchsuchen" tab with bout list, metric table, distance chart, annotations
- **NEW:** Output file list (PDF, HD video, highlight reel) with path + size
